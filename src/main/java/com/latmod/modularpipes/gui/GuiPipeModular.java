...
public class GuiPipeModular extends GuiContainer
{
	...
	private class ButtonFilter extends GuiButton
	{
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
		{
			if (visible)
			{
				...
				if (!container.pipe.inventories[id].module.isEmpty())
				{
					zLevel = 100F;
					itemRender.zLevel = 100F;
					GlStateManager.enableDepth();
					RenderHelper.enableGUIStandardItemLighting();
					itemRender.renderItemAndEffectIntoGUI(mc.player, container.pipe.inventories[id].module, x, y);
					itemRender.renderItemOverlayIntoGUI(fontRenderer, container.pipe.inventories[id].module, x, y, "");
	...
	public void renderHoveredToolTip(int x, int y)
	{
		...
		for (GuiButton button : buttonList)
		{
			...
			{
				ItemStack stack = container.pipe.inventories[button.id].module;
