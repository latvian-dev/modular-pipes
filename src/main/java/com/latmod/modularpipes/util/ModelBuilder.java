...
public class ModelBuilder
{
    ...
    private static final ModelRotation[] ROTATION_FROM_FACING = {ModelRotation.X0_Y0, ModelRotation.X180_Y0, ModelRotation.X90_Y180, ModelRotation.X90_Y0, ModelRotation.X90_Y90, ModelRotation.X90_Y270};
    ...
    public ModelBuilder(EnumFacing facing)
    {
        quads = new ArrayList<>();
        setRotation(facing);
    }
    ...
    public void append(ModelBuilder builder)
    {
        quads.addAll(builder.quads);
    }
    ...
    {
        rotation = ROTATION_FROM_FACING[facing.ordinal()];
    }

    public void setShading(boolean v)
    {
        shading = v;
    }
    ...
    public void addCube(float fromX, float fromY, float fromZ, float toX, float toY, float toZ, Function<EnumFacing, TextureAtlasSprite> sprites)
    {
        for(EnumFacing facing : EnumFacing.VALUES)
        {
            addQuad(fromX, fromY, fromZ, toX, toY, toZ, facing, sprites.apply(facing));
        }
    }
    ...
    public void addQuad(float fromX, float fromY, float fromZ, float toX, float toY, float toZ, EnumFacing face, @Nullable TextureAtlasSprite sprite)
    {
        if(sprite != null)
        {
            ...
            quads.add(BAKERY.makeBakedQuad(new Vector3f(fromX, fromY, fromZ), new Vector3f(toX, toY, toZ), new BlockPartFace(face, -1, "", new BlockFaceUV(uv, 0)), sprite, face, rotation, null, false, shading));
        }
    }
}